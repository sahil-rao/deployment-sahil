...
def execute(tenantid, msg_dict):
    ...
    if 'next' not in msg_dict:
        col_data = entities.find({"etype":"SQL_TABLE_COLUMN"}, {"eid": 1, "name": 1}).sort("_id", 1).limit(PAGINATION_LIMIT)
    else:
        col_data = entities.find({"_id": {'$gt':ObjectId(msg_dict['next'])}, "etype":"SQL_TABLE_COLUMN"}, {"eid": 1, "name": 1}).sort("_id", 1).limit(PAGINATION_LIMIT)
    for ret_count, col in enumerate(col_data):
...
if __name__ == '__main__':
    ...
    print "Second Four"
    pprint.pprint(execute(sys.argv[1], {'since':'57c5d6efad9d8b693f5696a0'}))   
    '''
    print "Third Four"
    pprint.pprint(execute(sys.argv[1], {'since':'57c5d6eead9d8b693f569671'}))   
    print "Fourth Four"

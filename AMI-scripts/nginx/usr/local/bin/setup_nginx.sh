...
# Set up crontab that dynamically reconfigures upstreams
(crontab -l || true; echo "* * * * * /bin/bash /usr/local/bin/reload_nginx.sh >>/var/log/reload_nginx.log 2>&1") 2>&1 | grep -v "no crontab" | sort | uniq | crontab -

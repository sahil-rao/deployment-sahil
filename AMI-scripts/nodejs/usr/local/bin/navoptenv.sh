...
export AWSAccessKeyId=`curl --silent http://169.254.169.254/latest/meta-data/iam/security-credentials/${IAM_ROLE} | jq -r '.AccessKeyId'`
export AWSSecretAccessKey=`curl --silent http://169.254.169.254/latest/meta-data/iam/security-credentials/${IAM_ROLE} | jq -r '.SecretAccessKey'`
